---
name: restart-bot
description: claude-crew の launchd サービスを制御する（再起動・状態確認・ログ確認）。トリガーワード：「再起動」「restart」「ボットを再起動」「サービスを再起動」「bot restart」「起動して」「止めて」「ログ確認」。
---
...
```bash
launchctl unload ~/Library/LaunchAgents/ai.claude-crew.web.plist && \
launchctl load ~/Library/LaunchAgents/ai.claude-crew.web.plist
```

### 状態確認

```bash
launchctl list | grep claude-crew
# 出力例: 85735  0  ai.claude-crew.web
# 列: PID / 終了コード / ラベル
# PID が "-" なら停止中、数値なら起動中
```
